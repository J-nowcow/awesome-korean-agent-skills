[← 목록으로](../README.md)

# DevOps & 배포


<!-- CAT_STATS:START -->
> 📦 **15개 항목** · 자동 갱신: 2026-05-22
<!-- CAT_STATS:END -->
> CI/CD, 릴리즈 자동화, 모니터링, 세션 관리

## 🔧 Skills

| 이름 | 도구 | 설명 | 언어 | 레포 |
|------|------|------|------|------|
| ci-cd-patterns | CC | GitHub Actions, 배포 전략 패턴 | 한국어 | [claude-integration](https://github.com/m16khb/claude-integration/tree/main/plugins/full-stack-orchestration/skills/ci-cd-patterns) |
| auto-release-manager | CC | 모든 프로젝트 버전·릴리즈 자동화 | 영+한 | [claude-code-marketplace](https://github.com/Dev-GOM/claude-code-marketplace/tree/main/plugins/auto-release-manager/skills) |
| harness-audit | CC | hooks/skills/agents/rules 전체 건강도 | 한국어 | [my-claude-code-asset](https://github.com/jh941213/my-claude-code-asset/tree/main/skills/harness-audit) |
| harness-diagnostics | CC | 12원칙 기반 자가 진단·개선 | 한국어 | [my-claude-code-asset](https://github.com/jh941213/my-claude-code-asset/tree/main/skills/harness-diagnostics) |
| session-wrap | CC | 세션 종료 전 4개 병렬 서브에이전트 정리 | 영+한 | [claude-forge](https://github.com/sangrokjung/claude-forge/tree/main/skills/session-wrap) |

| terrashark | CC/OC | Terraform 코드의 환각 현상을 수정하는 Claude/Codex 스킬 | 영+한 | [terrashark](https://github.com/LukasNiessen/terrashark) |
## ⚡ Commands

| 이름 | 도구 | 설명 | 언어 | 레포 |
|------|------|------|------|------|
| checkpoint | CC | 작업 상태 save/restore/list/diff/delete | 영+한 | [claude-forge](https://github.com/sangrokjung/claude-forge/blob/main/commands/checkpoint.md) |
| pm2 | CC | PM2 서비스 명령 자동 생성 | 영+한 | [everything-claude-code](https://github.com/affaan-m/everything-claude-code/blob/main/commands/pm2.md) |
| save/resume-session | CC | 세션 상태 저장·복원 | 영+한 | [everything-claude-code](https://github.com/affaan-m/everything-claude-code/tree/main/commands) — save-session.md, resume-session.md |
| harness-audit | CC | 하네스 감사 스코어카드 | 영+한 | [everything-claude-code](https://github.com/affaan-m/everything-claude-code/blob/main/commands/harness-audit.md) |

| dotnet-cursor-rules | CX | .NET 프로젝트를 위한 Cursor AI 규칙 모음 | 영+한 | [dotnet-cursor-rules](https://github.com/Aaronontheweb/dotnet-cursor-rules) |
## 🪝 Hooks

| 이름 | 도구 | 설명 | 언어 | 레포 |
|------|------|------|------|------|
| hook-session-summary | CC | 파일 작업 추적·요약 리포트 | 영+한 | [claude-code-marketplace](https://github.com/Dev-GOM/claude-code-marketplace/tree/main/plugins/hook-session-summary) |
| hook-sound-notifications | CC | 이벤트별 오디오 알림 | 영+한 | [claude-code-marketplace](https://github.com/Dev-GOM/claude-code-marketplace/tree/main/plugins/hook-sound-notifications) |
| hook-complexity-monitor | CC | 코드 복잡도 모니터링 (14개 언어) | 영+한 | [claude-code-marketplace](https://github.com/Dev-GOM/claude-code-marketplace/tree/main/plugins/hook-complexity-monitor) |
| aws-wa-cost / operational-excellence | CC | AWS 비용·운영 검토 | 한국어 | [roboco-io/plugins](https://github.com/roboco-io/plugins/tree/main/plugins/security/skills) |
